 ...
 */
import { setAttribute, removeArrayItem, eventListener } from '../../../helpers';
...
const settingField = {
	...
	setAllFieldDefaultValue( fields = [] ) {
		Object.entries( this.getFieldSchema() ).forEach( ( schema ) => {
			if ( fields ) {
				if ( fields.includes( schema[ 0 ] ) ) {
					schema[ 1 ].forEach( ( field ) => {
	...
	setGroupFieldsVisibility( groups, visibility ) {
		if ( groups && visibility ) {
			groups.forEach( ( group ) => {
				setAttribute.elem(
					`[data-group-field="${ settingField.prefix }_${ group }"]`,
					'data-visible',
					visibility
				);
			} );
	 ...
	 */
	onChangeTypeField() {
		eventListener( 'change', this.type, ( e ) => {
	 ...
	 */
	onChangeStyleField() {
		eventListener( 'change', this.style, ( e ) => {
			const target = e.target;
			...
			if ( typeElem ) {
				...
				if ( [ 'button', 'color', 'image' ].includes( type ) ) {
					...
					settingField.setGroupFieldsVisibility( groups, 'no' );
					settingField.setGroupFieldsVisibility(
						fields,
						'custom' === style ? 'yes' : 'no'
					);
	 ...
	 */
	onChangeShapeField() {
		eventListener( 'change', this.shape, ( e ) => {
			const target = e.target;
			...
			if ( typeElem ) {
				...
				if ( [ 'button', 'color', 'image' ].includes( type ) ) {
					...
					if ( 'custom' === shape ) {
						setAttribute.elem(
							`[data-group-field="${ prefix }_border_radius"]`,
							'data-visible',
							'yes'
						);
						...
						if ( [ 'color', 'image' ].includes( type ) ) {
							setAttribute.elem(
								`[data-group-field="${ prefix }_size"]`,
								'data-visible',
								'no'
							);
							setAttribute.elem(
								`[data-group-field="${ prefix }_dimension"]`,
								'data-visible',
								'yes'
							);
						}
