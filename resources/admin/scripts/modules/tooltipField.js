 ...
 */
import { setAttribute, setValue, eventListener } from '../../../helpers/index.js';

/**
 * Internal Modules.
 */
import imagePickerModule from './imagePicker.js';
...
const tooltipField = {
	...
	setToDefault( action = 'set', prefix ) {
		if ( action && prefix ) {
			...
			setAttribute.elem(
				`[data-group-field="${ prefix }_content_text"]`,
				'data-visible',
				'no'
			);
			setAttribute.elem(
				`[data-group-field="${ prefix }_content_html"]`,
				'data-visible',
				'no'
			);
			setAttribute.elem(
				`[data-group-field="${ prefix }_content_image"]`,
				'data-visible',
				'no'
			);
	 ...
	 */
	onChangeTypeField() {
		eventListener( 'change', '.hvsfw-tooltip-field-type', ( e ) => {
			...
			if ( prefix && [ 'none', 'default', 'text', 'image', 'html' ].includes( type ) ) {
				...
				setAttribute.elem(
					`[data-group-field="${ prefix }_content_${ type }"]`,
					'data-visible',
					'yes'
				);
			}
